...
public class DefaultContractEventDetailsFactory implements ContractEventDetailsFactory {
    ...
    public DefaultContractEventDetailsFactory(EventParameterConverter<Type> parameterConverter, Node node,
            String networkName) {
    ...
    @Override
    public ContractEventDetails createEventDetailsByEventFilter(List<ContractEventFilter> eventFilterList, Log log) {
        for (ContractEventFilter eventFilter : eventFilterList) {
            if(!isContractFilterMachLog(eventFilter, log)){
                continue;
            }
            final ContractEventSpecification eventSpec = eventFilter.getEventSpecification();
            final ContractEventDetails eventDetails = new ContractEventDetails();
    ...
    private List<EventParameter> typeListToParameterList(List<Type> typeList) {
        ...
        return typeList.stream().map(type -> parameterConverter.convert(type)).collect(Collectors.toList());
    }
    ...
    private List<Type> getNonIndexedParametersFromLog(ContractEventSpecification eventSpec, Log log) {
        ...
        return FunctionReturnDecoder.decode(log.getData(),
